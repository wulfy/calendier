...
module.exports = {
   ...
   getFirstDayMomentForMonth = (monthNumber) => {
    var format = "DD/MM/YYYY HH:mm:ss";
    return moment('01/'+monthNumber+'/2016 01:00:00',format);
  }
   getReservationByMonthDays = (reservations,searchedMonth) => {
  ...
  getDuree = (periodejour) =>{
    var durees = periodejour.split(";");
    return (durees.length % 2 == 0)? durees : -1;
  }

  getNbCreneaux = (endDate,startDate,dureeCreneau) => {
    return (endDate-startDate) / (dureeCreneau*60*1000);
  }


  getCreauxLibresHelper = (dateDay,reservationforday,periodes,duree_reservee,clickable) => {
  ...
  getMounthFreeCrenaux = (searchedMonth,reservations,periodes,duree_reservee,forcedStartDate) =>{
  ...
  mylog("getMounthFreeCrenaux " + realMonth + " " + searchedMonth);
    ...
    while(startDate.month()+1 == realMonth && watchDog <=32)
    {
      mylog("in");
          ...
          currentFree=getCreauxLibresHelper(startDate,reservations,periodes,duree_reservee,true);
  ...
  getFreeTimeForDuration = (startDate,endDate,dayreservations,duree,clickable) => {
